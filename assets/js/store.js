document.addEventListener("alpine:init", async () => {
	...
	Alpine.store("blocklist", {
		...
		stringify() {
			if (this.item && this.blockItem)
				//in a loop from template rendering.
				return JSON.stringify({
					blockItem: {...this.blockItem, ...{group: this.item.key}},
				});
		},
	});
	...
	Alpine.store("zoom", {
		...
		zoom(zoomIndex = 4, val = "") {
			this.currentZoomLevel =
				val === "" ? this.zoomLevels[zoomIndex] : val;
	...
	Alpine.store("url", {
		...
		async fetchCampaignUrls(campaignId) {
			...
			let resp = await automationHttpRequest(
				`${CAMPAIGN_TEMPLATE_URLS_FETCH_URL}?campaign_id=${campaignId}`,
				"GET"
			);

			if (resp.success) this.items = resp.data;
...
function campaignSelectionChange(element) {
	...
	const event = new CustomEvent("campaign-change", {
		detail: {campaignId, urlId},
	});
	// Dispatch the event.
	...
	return {
		...
		pairAdd(event) {
			...
			parent
				...
				.insertAdjacentHTML(
					"beforeend",
					parent.querySelector("#inputReps").innerHTML
				);
		},
	};
}
