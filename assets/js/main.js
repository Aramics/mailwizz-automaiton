document.addEventListener("DOMContentLoaded", function () {
	...
	function validateBlock(block, first, parent) {
		...
		if (!parent) {
			...
			if (blockGroup != BLOCK_GROUPS.TRIGGER) {
				setTimeout(() => {
					flowy.deleteBlocks();
				}, 0);

				notify("Only triggers can be used as the first block.");
		...
		if (parent) {
			...
			if (parentType == blockType) {
				if (parentType != BLOCK_GROUPS.ACTION) {
					notify("Cant have same non action block as a direct child");
					return false;
				}
			}
			...
			const yesNo = ["yes", "no"];
			const parentIsYesNo = yesNo.includes(parentType);
			const blockIsYesNo = yesNo.includes(blockType);

			//only logic should follow logics other than yes or no
			if (parentGroup == BLOCK_GROUPS.LOGIC) {
				if (!parentIsYesNo && blockGroup != BLOCK_GROUPS.LOGIC) {
					notify("Only logic should be folled by a logics block");
					return false;
				}
			}

			if (blockGroup == BLOCK_GROUPS.LOGIC) {
				//Non logic blocks should not be followed by yes or no
				if (parentGroup != BLOCK_GROUPS.LOGIC && blockIsYesNo) {
				...
				if (
					parentGroup == BLOCK_GROUPS.LOGIC &&
					!parentIsYesNo &&
					!blockIsYesNo
				) {
					notify(
	...
	/////////////////////// Canvas init, import and export //////////////////

	const initFlowBuilder = () => {
		flowy(canvas, onDrag, onRelease, onSnap, onRearrange, 20, 50);

		//get flow for the automation from server
		if (typeof AUTOMATION_DETAILS === "undefined") {
			notify("Automation details not found!");
			return;
		}

		if (AUTOMATION_DETAILS.canvas_data) {
			if (typeof AUTOMATION_DETAILS.canvas_data != "object")
				AUTOMATION_DETAILS.canvas_data = JSON.parse(
					AUTOMATION_DETAILS.canvas_data
				);

			importFlow({
				content: AUTOMATION_DETAILS.canvas_data.blocks_html_compressed,
			});
		}
	};
	...
	const saveFlow = async () => {
		let automation = Alpine.store("automation");

		//export data in object
		let output = exportFlow({exportToBox: false}) ?? {};

		//save to backend
		const resp = await automationHttpRequest(AUTOMATION_SAVE_URL, "POST", {
	...
	// @TODO: see template saving scheme.
	const exportFlow = ({exportToBox = true}) => {
		deselectBlock();
		...
		try {
			...
			if (!output) {
				throw new Error("Canvas appear to be empty!");
			}
			...
			let outputCompressed = compressData(output);

			if (!exportToBox)
				return {
					blocks_html_compressed: outputCompressed,
					blocks: output.blocks,
				};

			// show madal with text box, paste the output into textarea
			let exportBox = document.getElementById("export-box");
			if (!exportBox) throw new Error("Export box can not be find");

			exportBox.value = "";
			exportBox.value = outputCompressed;
		} catch (error) {
...
/////////////////////////****Network *////////////////////////
/**
 * Fetch wrapper for making http calls
 *
 * @param {string} url The url
 * @param {string} method Request method
 * @param {object} data data to be posted to endpoint
 * @returns
 */
async function automationHttpRequest(url, method, data) {
	let body = null;

	if (method.toUpperCase() == "POST") {
		let csrf_token_name = document.querySelector(
