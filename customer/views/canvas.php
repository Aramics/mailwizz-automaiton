...
<body :class="{'left-card-closed': !$store.navs.leftnav}">
    <header id="navigation" class="flex">
        <div id="leftside">
            <div id="details">
                <a href="javascript:window.history.back();" id="back"><img src="<?= $assets_dir; ?>/images/arrow.svg"
                        alt="back"></a>
                <div id="names">
                ...
                <div x-show="$store.canvas.selectedBlock">
                    ...
                    <template x-component="list">
                        <div x-data="{ ...$el.parentElement.props }"
                            x-on:global-lists-load.window="if(typeof global_list_key !== 'undefined') items=$event.detail[global_list_key]">
                    ...
                    <template x-component="mail-list">
                        <x-list
                            x-data="{ ...MailListComponent(), ...$el.parentElement.props, ...{global_list_key:'mail_list'} }">
                        </x-list>
                    </template>
                    ...
                    <template x-component="interval">
                        <div class="flex" x-data="{ 
                                ...{intervals: INTERVALS, label:'', interval_value: 1, interval_unit_in_secs: INTERVALS[0].key}, 
                            ...$el.parentElement.props }">
                            <input :name="name" :value="interval_value * interval_unit_in_secs" type="hidden" />
                            <label>
                                <span x-text="label"></span>
                                <input type="number" step="1.0" :name="interval_name" min="1" value="1"
                                    x-model="interval_value" />
                            </label>

                            <select :name="unit_name" x-model="interval_unit_in_secs">
                                <template x-for="item in intervals">
                                    <option :value="item.key" x-text="item.label" x-bind:title="item.label">
                                    </option>
                                </template>
                            </select>
                        </div>
                        ...
                        <div class="pairs-wrapper" x-data="{ ...InputPairRepeat(), ...$el.parentElement.props}">
                            ...
                            <template id="inputReps">
                                <div class="flex mt-10">
                                    <input type="text" value="" name="fields[]" placeholder="field" data-exclude />
